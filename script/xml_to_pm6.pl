#!/usr/bin/perl -w
#
# builds the _Perl 6_ Astro::Constants modules from an xml definition file
# Boyd Duffee, Apr 2018
#
...
for my $constant ( $xml->getElementsByTagName('PhysicalConstant') ) {
	...
	if ( $constant->getChildrenByTagName('alternateName') ) {
		for my $node ( $constant->getChildrenByTagName('alternateName') ) {
			...
			write_constant($ac_fh, ($values->{mks} || $values->{value}), $alternate) 
	...
	write_constant($ac_fh, ($values->{mks} || $values->{value}), $long_name, $short_name) 
			if $values->{mks} || $values->{value};
...
sub write_constant {
	...
	say $fh "my constant $long_name is export = $value;";
}
